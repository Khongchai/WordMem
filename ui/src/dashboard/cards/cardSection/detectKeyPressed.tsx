...
function manageDownKeys(
  undoKey: { z: boolean },
  redoKey: { y: boolean },
  ctrlPressed: boolean,
  setMutableVocabList: any
) {
...
async function handleUndoRedo(
  ...
  if (ctrlPressed) {
    ...
    let fromBackend: any = await detectChangedObjects(
      stateBeforeUndoORRedo,
      StateAfterUndoORRedo
    );

...
export default function DefinitionsFromAPI(props)
{
        ...
        switch(dictionary)
        {
            case("cambridge"):
                ...
                .then(arrayOfDefinitions => {
                    props.setMeaning(getDefinitionsAsHTML(arrayOfDefinitions, "cambridge"))
                });
                break;
            case("oxford"):
                oxfordDefinitionAPI(selectedWord)
                .then(arrayOfDefinitions => {
    ...
    function getDefinitionsAsHTML(arrayOfDefinitions, dictionary)
    {
        ...
        return(
            <div className="definition-block">
                <DefinitionsList arrayOfDefinitions={arrayOfDefinitions} url={url} dictionary={dictionary} />
            </div>
    ...
    function DefinitionsList(props)
    {
        ...
        {
            return <h3 style={{color: "#F17300"}}>No definitions for this word available in {capFirst(props.dictionary)} Dictionary.</h3>
        }
        else
        {
            return (
                <ol>
                    {props.arrayOfDefinitions.map(definition => <li className="definition">{definition}</li>)}
                    <a href={props.url} target="_blank">Go to Dictionary</a>
                </ol>
            )
        }
    }

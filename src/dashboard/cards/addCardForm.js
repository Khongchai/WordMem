...
export default function AddCardForm(props) {
  ...
  async function addCardToCardList(newCard) {
    let token = getToken();
    addVocab(token, newCard).then((newCardList) => {
      if (newCardList !== 409) {
        setVocabAndAddAnimation(newCardList);
      } else {
        showToast("Word already exists", "red");
      }
    });
  }
    ...
    >
      ...
      >
        <input
          ...
          name="word"
          required
          placeholder="Word; example: 'Happiness'"
        />
        ...
        <textarea
          ...
          name="meaning"
          className="spacious-window"
          placeholder="Meaning; example: 'A type of good feeling'"
          required
        />
